...
- In GitHub Actions, store `HOPSWORKS_API_KEY` and `HOPSWORKS_PROJECT` as Repository Secrets (Settings → Secrets and variables → Actions) — never in the repo.
...
- **Data drift** — KS test + PSI per feature (recent window vs reference); a feature is flagged DRIFT only when both agree (PSI ≥ 0.2 and KS p-value < 0.05).
...
Both are stateless: they restore state from Hopsworks at the start, so no data or model
